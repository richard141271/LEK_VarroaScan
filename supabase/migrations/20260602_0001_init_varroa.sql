...
create policy if not exists varroa_submissions_insert_anyone
  on public.varroa_submissions
  for insert
  to anon, authenticated
  with check (true);

create policy if not exists varroa_submissions_admin_select
  on public.varroa_submissions
  for select
  to authenticated
  using (exists (select 1 from public.varroa_admins a where a.user_id = auth.uid()));

create policy if not exists varroa_submissions_admin_update
  on public.varroa_submissions
  for update
  to authenticated
  using (exists (select 1 from public.varroa_admins a where a.user_id = auth.uid()))
...
create policy if not exists varroa_admins_admin_select
  on public.varroa_admins
  for select
  to authenticated
  using (exists (select 1 from public.varroa_admins a where a.user_id = auth.uid()));

create policy if not exists varroa_admins_admin_insert
  on public.varroa_admins
  for insert
  to authenticated
  with check (exists (select 1 from public.varroa_admins a where a.user_id = auth.uid()));

create policy if not exists varroa_labels_admin_all
...
create policy if not exists varroa_models_admin_all
...
create policy if not exists storage_varroa_upload_anyone
  on storage.objects
  for insert
  to anon, authenticated
  with check (bucket_id = 'varroa-submissions' and name like 'submissions/%');

create policy if not exists storage_varroa_admin_read
  on storage.objects
  for select
  to authenticated
  using (
...
create policy if not exists storage_varroa_admin_update
  on storage.objects
  for update
  to authenticated
  using (
...
create policy if not exists storage_varroa_admin_delete
  on storage.objects
  for delete
  to authenticated
  using (
